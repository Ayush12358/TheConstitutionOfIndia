...
// its predecessor and attribute every changed line (del + add) that starts an
// article heading to that version's amendment. `from` values ascend in file
// order, so the lists come out ascending; the first entry (the original text,
// from 0) has no predecessor and is skipped. A key is recorded once per
